(ns finanalyst.actor
  "FinancialAnalystsActor — the ISCO-08 2413 community finanalyst actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one finanalyst operation request
  ...
  The unconditional invariant: the FinancialAnalystsAdvisor can never
  directly commit a record the FinancialAnalystsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [finanalyst.advisor :as advisor]
            [finanalyst.governor :as governor]
            [finanalyst.store :as store]))

(defn build-graph
  "Build a compiled FinancialAnalystsActor graph. `store` implements
  `finanalyst.store/Store`. `advisor` implements
  `finanalyst.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :security-id (:security-id proposal)
                                    :payload proposal}]
